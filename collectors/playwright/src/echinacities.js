...
const BASE_URL = "https://jobs.echinacities.com";
...
async function extractListingJobs(page) {
  return page.evaluate(() => {
    ...
    const cards = Array.from(document.querySelectorAll(".job-list .job-item, .job-item, .search-result .item, li, article"));
    const jobs = [];
    ...
    for (const card of cards) {
      ...
      const institution =
        clean(card.querySelector(".company, .company-name, .school, .employer")?.textContent) || "";
      const location =
      ...
      const snippet =
        clean(card.querySelector(".desc, .summary, .job-desc, .content")?.textContent) || "";

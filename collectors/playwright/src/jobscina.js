...
const BASE_URL = "https://jobscina.com";
...
async function extractListingJobs(page) {
  return page.evaluate(() => {
    ...
    const cards = Array.from(
      document.querySelectorAll(".job-item, .job-list-item, .search-item, .position-item, li, article")
    );
    const jobs = [];
...
export default async function collect() {
  ...
  try {
    for (const keyword of SEARCH_KEYWORDS) {
      const searchUrl = `${BASE_URL}/jobs/list?s=${encodeURIComponent(keyword)}`;
